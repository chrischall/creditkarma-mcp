...
- **Persisted credentials**: `saveSession()` (in `src/session.ts`) writes the saved-session file at mode 0600 in a 0700 directory. Anything else handling secrets in this repo should match. The fetchproxy path doesn't persist anything — cookies are read into memory once per MCP run.
