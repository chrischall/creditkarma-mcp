...
- Credentials are stored only in your saved-session file (`~/.creditkarma-mcp/session`), Claude config / `.env`, or your browser's cookie jar (fetchproxy path)
- The saved-session file is written at mode 0600 (owner read/write only), in a 0700 directory, by `ck_set_session`
