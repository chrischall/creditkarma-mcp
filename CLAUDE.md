...
- **`No query found` is CK's fault, not ours**: the gateway rejects roughly a third to a half of well-formed requests with `HTTP 400 {"message":"No query found"}`. Measured 2026-08-01 against a live account, and it is **not** a request-shape problem — falsified: query size (a 25-byte `query Ping { __typename }` fails at the same rate as the 98KB document), persisted queries (ad-hoc queries execute fine, so the gateway is not APQ-only despite the APQ-flavoured wording), the `ck-*` client headers, session/Akamai (`bm_sz`/`_abck`) cookies, and request pacing (250ms vs 3s: 10/16 vs 8/16 failures). Successes and failures both reach the origin with identical headers apart from `connection: close` on the failure. Alternative transports can't be A/B'd from Node — Akamai 302s `node:https` outright and refuses ALPN for `node:http2` — so undici's default HTTP/1.1 is the only path that works at all. `fetchPage()` therefore retries this **one** signature up to `NO_QUERY_FOUND_ATTEMPTS` times (`isNoQueryFound` matches status 400 AND the exact `message` value, so genuine 400s still fail on the first attempt). Don't "fix" it with headers — an early single-shot A/B suggested `ck-client-name` was the cure and repeating it disproved that.
